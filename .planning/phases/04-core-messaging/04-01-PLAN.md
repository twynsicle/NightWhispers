...
must_haves:
  truths:
    - "Messages persist to database and survive page refresh"
    - "Messages broadcast to room channel in real-time"
    - "Hook loads initial message history on mount"
    - "sendMessage function writes to DB and broadcasts"
  artifacts:
...
- Message type exported from supabase.ts
- sendMessage and sendBroadcastMessage exported from message-helpers.ts
- Database insert uses .select().single() to return created message
- Broadcast send() call present in both helpers
  </verify>
  <done>
...
- Cursor-based pagination prevents duplicates when new messages arrive (RESEARCH.md Pitfall 7)
  ...
  <verify>
- TypeScript compiles without errors
- useMessages hook exported from src/hooks/useMessages.ts
- Hook loads initial messages using database query with conversation filter
- Broadcast subscription present with event: 'message'
- Cleanup function calls supabase.removeChannel(channel)
- sendMessage wrapper returns Promise&lt;Message&gt;
  </verify>
  <done>
