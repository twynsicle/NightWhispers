...
must_haves:
  truths:
    ...
    - "After leaving, player is redirected to home page and cannot access room without rejoining"
  artifacts:
...
# RoomPage loader includes participant.rooms relation
...
<task type="auto">
  <name>Task 3: Integrate settings menu into PlayerChatView and fetch roomCode</name>
  <files>
    src/components/PlayerChatView.tsx
  </files>
  <action>
    ...
    **Add roomCode state and fetch:**
    - Import supabase client
    - Add state: `const [roomCode, setRoomCode] = useState<string | null>(null)`
    - Add useEffect to fetch room code on mount:
      ```typescript
      useEffect(() => {
      ...
      ```
    - This query uses the same pattern as RoomPage loader (selects rooms relation)
    ...
    **Add settings button:**
    ...
    - Render PlayerSettingsMenu at bottom of component:
      - Props: roomCode={roomCode || ''}, participantId, opened=settingsOpened, onClose={() => setSettingsOpened(false)}
      - Only render when roomCode is loaded (conditional on roomCode !== null)
    ...
    **Note on redirect behavior:**
    Existing RoomPage kicked detection (lines 116-144 in current RoomPage.tsx) subscribes to postgres_changes on participants.is_active. When leaveRoom sets is_active = false, RoomPage's subscription will detect the update and trigger redirect to home page. No additional redirect logic needed in this task.
  </action>
  <verify>
    Run `npx tsc --noEmit` to verify TypeScript compiles.
    Verify PlayerChatView imports PlayerSettingsMenu.
    Verify roomCode state and fetch query exist.
    Verify settings button renders in header.
    Run `npm run dev` to check UI loads.
  </verify>
  <done>
    PlayerChatView has roomCode state fetched from participants.rooms relation.
    Settings button renders in header.
    Button opens PlayerSettingsMenu on click.
    Room code and participant ID passed to menu.
    Settings integration complete.
    Redirect behavior handled by existing RoomPage postgres_changes subscription.
  </done>
</task>
...
- [ ] PlayerChatView has roomCode fetch query
...
- Redirect to home after leaving (handled by existing RoomPage subscription)
