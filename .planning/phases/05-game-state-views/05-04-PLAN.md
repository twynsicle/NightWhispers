...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/pages/RoomPage.tsx"
      provides: "Updated to pass roomCode prop to PlayerChatView"
      min_lines: 415
  key_links:
    ...
    - from: "src/pages/RoomPage.tsx"
      to: "src/components/PlayerChatView.tsx"
      via: "passes roomCode prop to PlayerChatView"
      pattern: "roomCode=.*participant\\.rooms\\.code"
---
...
<task type="auto">
  <name>Task 3: Integrate settings menu into PlayerChatView and pass roomCode from RoomPage</name>
  <files>
    src/components/PlayerChatView.tsx
    src/pages/RoomPage.tsx
  </files>
  <action>
    ...
    **Add settings button:**
    - Add roomCode prop to PlayerChatView interface: `roomCode: string`
    ...
    **Pass roomCode to PlayerChatView:**
    - In the active game view Player section (lines 344-366), update PlayerChatView to receive roomCode prop
    - Add `roomCode={participant.rooms.code}` to PlayerChatView component
    - This provides PlayerChatView with room code to pass to PlayerSettingsMenu
  ...
  <verify>
    ...
    Verify PlayerChatView has roomCode prop in interface.
    Verify settings button renders in header.
    Verify RoomPage passes roomCode prop to PlayerChatView.
    Run `npm run dev` to check UI loads.
  </verify>
  <done>
    PlayerChatView has roomCode prop and settings button in header.
    Button opens PlayerSettingsMenu on click.
    RoomPage passes roomCode={participant.rooms.code} to PlayerChatView.
...
- [ ] PlayerChatView has roomCode prop
- [ ] RoomPage passes roomCode to PlayerChatView
