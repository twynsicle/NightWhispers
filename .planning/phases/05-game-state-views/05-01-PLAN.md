...
<task type="auto">
  ...
  <action>
    ...
    - Phase format: alternates Night/Day with incrementing numbers
    - Uses UPDATE query to rooms table: `supabase.from('rooms').update({ phase: newPhase }).eq('id', roomId)`
    - Cleans up subscription on unmount

    Update `src/lib/supabase.ts`:
    - Add helper function `getNextPhase(currentPhase: string): string` that parses "Night X" or "Day X" and returns next phase
    - Logic: "Night 1" → "Day 1", "Day 1" → "Night 2", "Night 2" → "Day 2", etc.
    - Export function for use in usePhase hook
  ...
  <verify>
    ...
    Verify getNextPhase exported from supabase.ts.
  </verify>
  <done>
...
- [ ] getNextPhase helper exists in supabase.ts
...
**PLAY-03 delivered:** Player sees current game phase in header
