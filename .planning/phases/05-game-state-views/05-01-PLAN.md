...
<task type="auto">
  ...
  <action>
    ...
    - Logic: "Night 1" → "Day 1", "Day 1" → "Night 2", "Night 2" → "Day 2", etc.
    ...
    - Provides advancePhase helper that increments phase (e.g., "Night 1" → "Day 1" → "Night 2")
...
<task type="auto">
  ...
  <action>
    ...
    - Include moon/sun icon based on Night/Day (🌙 for Night, ☀️ for Day)
...
<task type="auto">
  ...
  <action>
    ...
    **For Storyteller active view only:**
    ...
    - Use Stack to vertically arrange: PhaseHeader → PhaseControls → StorytellerDashboard

    **For Player active view:**
    - Only render PhaseHeader (no controls)
    - Use Stack to vertically arrange: PhaseHeader → PlayerChatView
    - **Explicit verification:** After integration, confirm PhaseHeader renders in Player view by checking that the component appears before PlayerChatView in the Player branch of the conditional (lines 344-365 in current RoomPage.tsx)
  ...
  <verify>
    ...
    Verify PhaseHeader also rendered for Player role (PLAY-03 requirement).
    Run `npm run dev` and navigate to active game to check UI loads.
  </verify>
  <done>
    RoomPage renders PhaseHeader for all participants in active game (both Player and Storyteller).
    RoomPage renders PhaseControls only for Storyteller in active game.
    Layout maintains mobile-first gothic theme.
    PLAY-03 requirement explicitly verified: Player role sees PhaseHeader.
  </done>
</task>
