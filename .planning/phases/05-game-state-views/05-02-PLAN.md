...
must_haves:
  truths:
    ...
    - "Custom status displays on player card for Storyteller only"
...
<task type="auto">
  <name>Task 3: Update ParticipantList to grey dead players and integrate status controls</name>
  <files>
    src/components/ParticipantList.tsx
    src/components/StorytellerDashboard.tsx
  </files>
  <action>
    ...
    - When participant.status === 'dead':
      ...
      - Add skull emoji (💀) next to name or as badge
    ...
    - Optionally add status indicator to player cards (dead badge or custom status preview)
...
2. Open conversation with a player
3. Verify "Mark as Dead" switch appears above chat
4. Toggle switch to mark player as dead
5. Verify player's avatar greys out in participant list for both Storyteller and Player
6. Verify skull emoji (💀) appears next to dead player's name
7. Enter custom status text "Poisoned" in input field
8. Wait 1 second (debounce)
9. Verify custom status appears on player card in Storyteller view
10. Refresh browser, verify status persists
11. Toggle switch back to alive, verify grey effect removes
</verification>
