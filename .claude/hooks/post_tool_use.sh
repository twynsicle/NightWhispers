...
# Only check TypeScript/TSX/JavssssssssssssssssaScript files
if [[ "$FILE_PATH" =~ \.(ts|tsx|js|jsx)$ ]]; then
  # Get relative path from workspace root
  RELATIVE_PATH="${FILE_PATH#/Users/stevenlemon/workspace/core/www/}"
  ...
  if [ $LINT_EXIT -ne 0 ]; then
    ...
    echo '{"action":"allow","context":"⚠️ Linting issues detected in '"$RELATIVE_PATH"'. Run `yarn lint:fix` to auto-fix."}'
    exit 0
  fi
fi