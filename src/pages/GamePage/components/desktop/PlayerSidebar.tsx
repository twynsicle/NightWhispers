...
export function PlayerSidebar({
  ...
  return (
    <Stack h="100%" gap={0}>
      ...
      {/* Reset button */}
      <Box p="md" className={styles.footer}>
        <Text
          size="sm"
          c="red"
          ta="center"
          ...
          onClick={onResetGame}
        >
          <IconRefresh size={14} className={styles.resetIcon} />
          Reset Game
        </Text>
      </Box>
    </Stack>
  ...
  return (
    ...
    >
      <Group justify="space-between" gap="sm" wrap="nowrap">
        <Group gap="sm" wrap="nowrap" className={styles.groupOverflow}>
          <Text size="lg" className={styles.avatarFlexShrink}>
            {icon === 'broadcast' ? '📢' : icon || '👤'}
          </Text>
          <Stack gap={2} className={styles.groupOverflow}>
            <Text size="sm" fw={500} className={styles.textEllipsis}>
              {name}
            </Text>
            {subtitle && (
              <Text size="xs" c="dimmed">
...
function SortableSidebarItem({
  ...
  // Dynamic styles from dnd-kit must remain inline (computed at runtime)
  const dragStyle = {
  ...
  return (
    <Card
      ...
      radius="md"
      onClick={onClick}
      className={selected ? styles.cardSelected : undefined}
    >
      <Group justify="space-between" gap="sm" wrap="nowrap">
        <Group gap="sm" wrap="nowrap" className={styles.groupOverflow}>
          <Text
            size="lg"
            className={`${styles.avatarFlexShrink} ${isDead ? styles.avatarDead : ''}`}
          >
            {player.avatar_id || '👤'}
          </Text>
          <Stack gap={2} className={styles.groupOverflow}>
